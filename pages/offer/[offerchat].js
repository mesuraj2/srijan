...
export default function offerchat({Offerdetail}) {
  ...
  const Joinchat= async()=>{
    ...
    const res =await fetch(`http://localhost:3001/api/chat/groupaddOffer`, {
        method: 'PUT', // or 'PUT'
        headers: {
...
Router.push('/chat')
  ...
  const handleSubmit= async (e)=>{
    ...
    const { data } = await axios.post(
      `http://localhost:3001/api/chat/offerchat`,
      {
        chatName: chatname,
...
export async function getServerSideProps(context) {
  const res =await fetch(`http://localhost:3001/api/offer/offerdetail`, {
    method: 'POST', // or 'PUT'
    headers: {
      'Content-Type': 'application/json',
    },
